    ...
    def thumbnail_all(self, percent=0.5):
        ...
        for file in self.files:
            if re.match('(.*jpg&)|(.*png)|(.*bmp)',file):
                ...
                target_filename=filename.split('.')[0]+'-thumbnail.'+filename.split('.')[1]
                self.thumbnail(file,target_filename,percent=0.5)
            else:
                # print('没有符合的文件')
                mylog.error('没有符合的文件')
...
def main():
    imageutil=ImageUtils(r'd:\Documents and Settings\yanchunwei\桌面\自动化保全\测试用例截图',r'd:\Documents and Settings\yanchunwei\桌面\自动化保全\用例截图')
    imageutil.thumbnail_all(percent=0.5)
