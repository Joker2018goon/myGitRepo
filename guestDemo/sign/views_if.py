...
def add_event(request):
    ...
    if eid == '' or name == '' or limit == '' or address == '' or start_time == '':
        return JsonResponse({'status': 10021, 'message': 'parametet error'})
    ...
    try:
        # start_time = datetime.datetime.strptime(start_time,'YYYY-MM-DD HH:MM:SS')
...
def add_guest(request):
    ...
    print(eid)
    print(event_time)
    print(Event.objects.get(id=eid).name)
    '''
    etime = str(event_time).split('+')[0]
    d_time = time.strptime(str(etime), "%Y-%m-%d %H:%M:%S")
    e_time = int(time.mktime(d_time))
...
def user_sign(request):
    ...
    event_time = Event.objects.get(id=eid).start_time  # 发布会时间
    # etime = str(event_time).split('+')[0]
    timeArray = time.strptime(str(event_time).split('+')[0], "%Y-%m-%d %H:%M:%S")
